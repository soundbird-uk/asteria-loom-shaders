...
/*
 composite13 (fragment) — BLOOM pyramid, UPSAMPLE level 2 (U2 = L2 + tent(U3)).

 Real dual-filter tent-cascade step (full chain in lib/bloom.glsl): 3x3-tent
 upsamples the coarser, already-accumulated tile U3 and ADDS it onto this
 level's own downsampled tile L2, overwriting tile L2 in place with U2. The
 tent taps are clamped inside tile L3 (no cross-tile bleed); this level's own
 value is read byte-exact from the pixel being written (texelFetch), which under
 the Iris flip returns the pre-flip L2 the downsample chain stored.
 ...
 Gated `program.composite13.enabled = BLOOM`. Sampler count: 1 (colortex9).
*/
...
void main() {
    ...
    // This level's own downsampled value (pre-flip tile L2 at this exact texel).
    vec3 base = texelFetch(colortex9, px, 0).rgb;

    // Tent-upsample the coarser accumulated tile U3. One source (L3) texel is
    // 2^3/screenRes in its local UV; AL_BLOOM_TENT_RADIUS widens the tent.
    vec2 sLocal = AL_BLOOM_TENT_RADIUS * exp2(float(3)) * atlasTexel;
    vec3 up = alBloomTentTile(colortex9, 3, localUV, sLocal, atlasTexel);
