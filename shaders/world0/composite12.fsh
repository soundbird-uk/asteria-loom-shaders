...
/*
 composite12 (fragment) — BLOOM pyramid, UPSAMPLE level 3 (U3 = L3 + tent(U4)).

 Real dual-filter tent-cascade step (full chain in lib/bloom.glsl): 3x3-tent
 upsamples the coarser, already-accumulated tile U4 and ADDS it onto this
 level's own downsampled tile L3, overwriting tile L3 in place with U3. The
 tent taps are clamped inside tile L4 (no cross-tile bleed); this level's own
 value is read byte-exact from the pixel being written (texelFetch), which under
 the Iris flip returns the pre-flip L3 the downsample chain stored.
 ...
 Gated `program.composite12.enabled = BLOOM`. Sampler count: 1 (colortex9).
*/
...
void main() {
    ...
    // This level's own downsampled value (pre-flip tile L3 at this exact texel).
    vec3 base = texelFetch(colortex9, px, 0).rgb;

    // Tent-upsample the coarser accumulated tile U4. One source (L4) texel is
    // 2^4/screenRes in its local UV; AL_BLOOM_TENT_RADIUS widens the tent.
    vec2 sLocal = AL_BLOOM_TENT_RADIUS * exp2(float(4)) * atlasTexel;
    vec3 up = alBloomTentTile(colortex9, 4, localUV, sLocal, atlasTexel);
