...
/*
 composite8 (fragment) — BLOOM pyramid, DOWNSAMPLE level 5 (from level 4).

 Real dual-filter pyramid step (full chain in lib/bloom.glsl): builds tile L5
 of the colortex9 atlas from the PREVIOUS, coarser-by-one tile L4 with a 13-tap
 ...
 downsample — L5 depends only on L4, never on colortex0 or hardware mips.
 ...
 Gated `program.composite8.enabled = BLOOM`. Sampler count: 1 (colortex9).
*/
...
void main() {
    ...
    // One source-tile (L4) texel in source-tile-local UV. Tile L4 resolution
    // is screen/2^4, so its local texel = 2^4/screenRes.
    vec2 atlasTexel = 1.0 / vec2(viewWidth, viewHeight);
    vec2 dLocal = exp2(float(4)) * atlasTexel;

    vec3 b = alBloomDownsampleTile(colortex9, 4, localUV, dLocal, atlasTexel);
