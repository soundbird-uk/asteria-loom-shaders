...
/*
 composite7 (fragment) — BLOOM pyramid, DOWNSAMPLE level 4 (from level 3).

 Real dual-filter pyramid step (full chain in lib/bloom.glsl): builds tile L4
 of the colortex9 atlas from the PREVIOUS, coarser-by-one tile L3 with a 13-tap
 ...
 downsample — L4 depends only on L3, never on colortex0 or hardware mips.
 ...
 Gated `program.composite7.enabled = BLOOM`. Sampler count: 1 (colortex9).
*/
...
void main() {
    ...
    // One source-tile (L3) texel in source-tile-local UV. Tile L3 resolution
    // is screen/2^3, so its local texel = 2^3/screenRes.
    vec2 atlasTexel = 1.0 / vec2(viewWidth, viewHeight);
    vec2 dLocal = exp2(float(3)) * atlasTexel;

    vec3 b = alBloomDownsampleTile(colortex9, 3, localUV, dLocal, atlasTexel);
