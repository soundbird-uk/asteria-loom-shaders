...
/* =========================================================================
   ADVANCED TIER (Phase 6 — Windows/Linux only; compiles OUT on macOS)
   -------------------------------------------------------------------------
   The advanced tier adds GPU-compute effects that fundamentally CANNOT exist on
   the macOS GL 4.1 path (no compute shaders, SSBOs or custom images). They are
   ALL gated behind the derived macro AL_ADVANCED_TIER, which is defined ONLY
   when the user opts in (ADVANCED_TIER) AND Iris reports every capability they
   need. On macOS none of those IRIS_FEATURE_* flags exist, so AL_ADVANCED_TIER
   is never defined and every advanced branch compiles away — the Mac build is
   byte-for-byte the tier-6-absent build (verified: the CI `mac`/`mac-hw` targets
   define none of these flags, so the gate is proven to compile out).

   VERIFICATION CEILING (honest note): these features are validated by the CI
   compile gate for SYNTAX only (glslang, advanced target). There is no GPU in
   CI and the maintainer's machine is macOS, so on-device behaviour of the
   compute passes has NOT been runtime-verified — treat them as syntax-complete
   scaffolding for compute-capable machines, not field-hardened.
   ========================================================================= */

// Master advanced-tier opt-in (GUI). Default ON so compute-capable machines get
// the tier; harmless on macOS (the IRIS_FEATURE_* gate below is never satisfied
// there). Windows/Linux users can turn it off to force the portable path.
#define ADVANCED_TIER // [ADVANCED_TIER]

// Derived gate — the ONLY thing the shaders test. Requires the opt-in plus the
// three compute capabilities. `#if defined` (not #ifdef) so all conditions read
// in one expression. Never define AL_ADVANCED_TIER directly anywhere else.
#if defined ADVANCED_TIER && defined IRIS_FEATURE_COMPUTE_SHADERS && defined IRIS_FEATURE_SSBO && defined IRIS_FEATURE_CUSTOM_IMAGES
    #define AL_ADVANCED_TIER
#endif

// --- Compute histogram auto-exposure (advanced only) ----------------------
// On the advanced tier the whole-screen average luminance that drives auto-
// exposure is metered from a real luminance HISTOGRAM built in a compute pass
// (world0/composite5_a/_b.csh) rather than a single deep-mip average. A
// histogram rejects tiny bright/dark outliers (the sun disc, a torch, a cave
// mouth) that skew a mip average, so exposure tracks the scene the eye actually
// adapts to. The metered value lands in a colortex5 scratch texel that the
// portable composite5.fsh reads (see its AL_ADVANCED_TIER branch); everything
// downstream (adaptation, clamp, AgX) is shared with the Mac path.
#define AL_HISTO_BINS        128     // luminance buckets (power of two, <=256)
#define AL_HISTO_LOG_MIN    -8.0     // log2 luminance mapped to bin 0 (~0.0039)
#define AL_HISTO_LOG_MAX     4.0     // log2 luminance mapped to the top bin (16.0)
#define AL_HISTO_LOW_CLIP    0.50    // discard this fraction of darkest samples
#define AL_HISTO_HIGH_CLIP   0.90    // ...and everything above this cumulative frac
// colortex5 scratch texel that carries the compute-metered luminance to the
// fragment pass (its .a; (0,0).a is the adapted-exposure slot, so use (1,0)).
#define AL_HISTO_TEXEL       ivec2(1, 0)
